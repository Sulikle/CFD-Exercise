function [n,Ul2errors,Vl2errors]=subDGP0P1plusDGP0(Unit,CFL,endtau,tol,belta,nsdv,nexplicit)
%% Preproceeding
...
%% Explicit
if nexplicit==1
         [Unumsolution,n]=P0P1plusP0Explicit(Unit,CFL,endtau,tol,Grid,Deltax,nsdv);
...
figure
 ...
 p=@(x)Unumsolution(1,k)+Unumsolution(2,k)*(x-xci)/Deltax(k);
 y=p(x);
...
ylabel('数值U','fontsize',14)
 title('DG(P0P1)+DG(P0) Explicit Euler数值解与解析解(U)','fontsize',16)
hold off
...
%  p=@(x)Unumsolution(2,k)/Deltax(k);
 y=[Unumsolution(2,k)/Deltax(k),Unumsolution(2,k)/Deltax(k)];
 plot(x,y,'-r^','linewidth',1.5);hold on
 H1=plot(x,y,'-r^','linewidth',1.5);hold on
 for k=2:numberx-1
     x=Grid(k):1*(Grid(k+1)-Grid(k)):Grid(k+1);
...
xlabel('位置x','fontsize',14)
ylabel('数值U','fontsize',14)
title('DG(P0P1)+DG(P0) Explicit Euler数值解与解析解(Ux)','fontsize',16)
hold off
...
xlabel('位置x','fontsize',14)
ylabel('数值U','fontsize',14)
 title('DG(P0P1)+DG(P0) TVDRK3 数值解与解析解(U)','fontsize',16)
hold off       
...
xlabel('位置x','fontsize',14)
ylabel('数值U','fontsize',14)
title('DG(P0P1)+DG(P0) TVDRK3 数值解与解析解(Ux)','fontsize',16)
hold off
...
[Unumsolution,n]=P0P1plusP0BDF1(Unit,CFL,endtau,tol,Grid,Deltax,nsdv);
Ul2errors=UL2errors(Unumsolution,Deltax,Grid);
Vl2errors=VL2errors(Unumsolution,Deltax,Grid);
%% Jacobi
if nsdv==1
...
xlabel('位置x','fontsize',14)
ylabel('数值U','fontsize',14)
 title('DG(P0P1)+DG(P0)Jacobi数值解与解析解(U)','fontsize',16)
hold off
...
xlabel('位置x','fontsize',14)
ylabel('数值U','fontsize',14)
title('DG(P0P1)+DG(P0) Jacobi数值解与解析解(Ux)','fontsize',16)
hold off
...
xlabel('位置x','fontsize',14)
ylabel('数值U','fontsize',14)
 title('DG(P0P1)+DG(P0) LUSGS数值解与解析解(U)','fontsize',16)
hold off
...
xlabel('位置x','fontsize',14)
ylabel('数值U','fontsize',14)
title('DG(P0P1)+DG(P0) LUSGS数值解与解析解(Ux)','fontsize',16)
hold off
