function [Unumsolution,n]=P0P1plusP0Explicit(Unit,CFL,endtau,tol,Grid,Deltax,nsdv)
dimention=2;
%%Pre-proceeding
...
W=[5/9,8/9,5/9];
  for ie=1:Unit
    ...
    for ig=1:3
        ...
        varphig=Uhold(1,ie)+B2g*Uhold(2,ie);
        Vg=Uhold(2,ie)/Deltax(ie);
        S1g=pi^2*sin(pi*xig);
        S2g=-Vg/Tr;
        F1g=-nu*Vg;
        F2g=-varphig/Tr;
...
%Rboundary
  for iface=2:numberx-1
    ...
    B2L=1/2;
    B2R=-1/2;
    varphiL=Uhold(1,ieL)+B2L*Uhold(2,ieL);
    varphiR=Uhold(1,ieR)+B2R*Uhold(2,ieR);
    VL=Uhold(2,ieL)/Deltax(ieL);
    VR=Uhold(2,ieR)/Deltax(ieR);
...
if nsdv==1
fprintf('DG(P0P1)+DG(P0) Explicit Euler 达到稳态所需要的时间是：%f 秒\n',n)
  elseif nsdv==2
    fprintf('DG(P0P1)+DG(P0) TVDRK3 达到稳态所需要的时间是：%f 秒\n',n)
end
