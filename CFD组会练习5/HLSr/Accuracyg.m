...
Grid(1,numberx)=endx;
g=@(x)x.^4+x.^3+x.^2+x+1;G=@(x)4*x.^3+3*x.^2+2*x+1;
...
%¶Ôg
for k=1:Unit
    Unumsolution(1,k)=(Grid(k+1)-Grid(k)+0.5*(Grid(k+1)^2-Grid(k)^2)+(Grid(k+1)^3-Grid(k)^3)/3+(Grid(k+1)^4-Grid(k)^4)/4+(Grid(k+1)^5-Grid(k)^5)/5)/(Grid(k+1)-Grid(k));
...
A=[(xci1-xci)^2/(2*(Grid(1+1)-Grid(1))^2)+1/24*((Grid(1+2)-Grid(1+1))^2/(Grid(1+1)-Grid(1))^2-1);
        (Grid(1+2)-Grid(1+1))*(xci1-xci)/(Grid(1+1)-Grid(1))^2;
        (xci1-xci)/(Grid(1+1)-Grid(1))^2];
b=[Unumsolution(1,1+1)-Unumsolution(1,1)-Unumsolution(2,1)*(xci1-xci)/(Grid(1+1)-Grid(1));
        Unumsolution(2,1+1)-Unumsolution(2,1)*(Grid(1+2)-Grid(1+1))/(Grid(1+1)-Grid(1));
...
k=numberx-1;
 ...
 A=[(xci2-xci)^2/(2*(Grid(k+1)-Grid(k))^2)+1/24*((Grid(k)-Grid(k-1))^2/(Grid(k+1)-Grid(k))^2-1);
        (Grid(k)-Grid(k-1))*(xci2-xci)/(Grid(k+1)-Grid(k))^2;
        (xci2-xci)/(Grid(k+1)-Grid(k))^2];
 b=[Unumsolution(1,k-1)-Unumsolution(1,k)-Unumsolution(2,k)*(xci2-xci)/(Grid(k+1)-Grid(k));
        Unumsolution(2,k-1)-Unumsolution(2,k)*(Grid(k)-Grid(k-1))/(Grid(k+1)-Grid(k));
        Unumsolution(2,k-1)/(Grid(k)-Grid(k-1))-Unumsolution(2,k)/(Grid(k+1)-Grid(k))];   
...
for K=1:numberx-1
   for i=1:3
       xi=(Grid(K+1)-Grid(K))/2*t(i)+0.5*(Grid(K+1)+Grid(K));
              ...
              fi=(xi^4+xi^3+xi^2+xi+1-(Unumsolution(1,K)+Unumsolution(2,K)*(xi-(Grid(K)+Grid(K+1))/2)/(Grid(K+1)-Grid(K))+Ureconstruct(1,K)*(0.5*((xi-(Grid(K)+Grid(K+1))/2)/(Grid(K+1)-Grid(K)))^2-1/24)))^2;
