...
for i=1:numberx-1
    ...
    Unumsolution(2,i)=(Grid(i+1)^2-Grid(i)^2)+(Grid(i+1)-Grid(i));
end
...
title('VR(DG(P0P2)+rDG(P0P1),NUG) F(x)=1+x+x^2','fontsize',16)
hold off
...
title('F(x)精度分析 DG(P0P2)+rDG(P0P1)','fontsize',16)
...
for i=1:numberx-1
    ...
    Unumsolution(2,i)= sin(pi*Grid(i+1))-sin(pi*Grid(i));
end
...
xlabel('Position x','fontsize',14)
ylabel('Numerical value','fontsize',14)
title('VR(DG(P0P2)+rDG(P0P1),NUG) G(x)=sin(pi*x)','fontsize',16)
hold off
...
xlabel('Log(1/DOF)','fontsize',14)
ylabel('Log(episilo)','fontsize',14)
title('G(x)精度分析 DG(P0P2)+rDG(P0P1)','fontsize',16)
