...
n=zeros(1,floor(endt/deltat));
 ...
 Uexasolution=zeros(2,numberx); Vnumsolution=zeros(3,numberx-1);
for k=1:numberx
...
for i=1:Unit
    ...
    LHS(dimention*(i-1)+3,dimention*(i-1)+3)=LHS(dimention*(i-1)+3,dimention*(i-1)+3)+(1/720*Deltax(i))/deltat;
end
...
for k=1:numberx-1
        ...
        W=[5/9,8/9,5/9];
          ...
          Vcurrent(2,k)=Vcurrent(2,k)+const*exp(-(xig-x0)^2/nu)*(-2*(xig-x0)/nu);
           ...
           Vcurrent(3,k)=(exp(-(xci-x0)^2/nu)*(-2*(xci-x0)/nu)^2+exp(-(xci-x0)^2/nu)*(-2/nu))*Deltax(k)^2;
